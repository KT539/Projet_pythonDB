...
def homepage_window(win):
    ...
    def switch_reservations():
        outer_frame.destroy()
        from Reservations_test import reservations_window # moved the import statement here on ChatGPT's suggestion, after experiencing circular import issues
    ...
    def switch_concerts():
        outer_frame.destroy()
        from Concerts_test import concerts_window # moved the import statement here on ChatGPT's suggestion, after experiencing circular import issues
        concerts_window(win)
    ...
    if admin_status == 1 :
        ...
        btn_visitors = tk.Button(inner_frame, text="Visitors", font=("Arial", 15), width=20,
                                 command=switch_visitors_admin)
    ...
    def switch_login():
        outer_frame.destroy()
        from Login_test import login_window  # moved the import statement here on ChatGPT's suggestion, after experiencing circular import issues
        login_window(win)
